...
02~07 분석은 공통 분류를 `sql/01_semantic_view.sql`의 `survey_semantic`에 한 번 정의하고, 분석별 추출·집계는 **노트북당 SQL 파일 1개**(`sql/NN_*.sql`)에 둔다. 노트북은 그 결과를 받아 **시각화·통계 검정·해석**을 담당한다. 한 `.sql` 파일 안에서 각 쿼리를 `-- name:` 마커로 구분하고(상단에 "Skills used" 헤더), 노트북은 쿼리를 이름으로 호출한다.

`survey_semantic`은 정제 원본 `survey`와 같은 **266행(응답자 1명당 1행)**을 유지한다. 플랫폼 사용자·구매자·RFM 대상은 행을 제거하지 않고 `is_platform_user`, `is_buyer`, `is_rfm_eligible` 플래그로 선택한다.
...
```text
survey (266행)
  → survey_semantic (266행 canonical semantic view)
    → 02~07 분석별 view/쿼리
    → v_tableau_main / v_tableau_channel
```

각 02~07 노트북의 setup 셀은 `survey_semantic`을 먼저 생성한 뒤 해당 노트북의 분석 view를 만든다. 따라서 `survey`가 적재된 clean DB라면 각 노트북을 독립적으로 `Restart & Run All` 할 수 있다. Tableau는 `survey_semantic` 생성 후 `tableau/tableau_views.sql`을 실행한다.
...
각 노트북을 `Restart & Run All` 하면 `load_queries`로 `sql/01_semantic_view.sql`과 `sql/NN_*.sql`을 읽어, canonical view → 분석별 뷰·테이블 순서로 만든 뒤 집계 쿼리를 이름으로 호출한다. DB 연결은 프로젝트 루트 `.env`. 통계 검정 방법론은 [METHODS.md](METHODS.md) 참조.
